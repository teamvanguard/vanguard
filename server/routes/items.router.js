  ...
  if(req.isAuthenticated()){
    pool.connect(function(errorConnectingToDatabase, client, done) {
      if (errorConnectingToDatabase) {
        ...
        if(req.user.role == constantModule.STORE_MANAGER_ROLE || req.user.role == constantModule.ADMIN_ROLE) {
          var queryText = 'SELECT items.id, items.item_name, items.item_description, items.pts_value, items.item_image, items.school_community, users.name, items.qty FROM items LEFT OUTER JOIN users ON users.id = items.last_edit_user_id ORDER BY id DESC';
